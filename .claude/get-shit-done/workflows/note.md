...
- **Global scope**: `/home/ubuntu/repos/lucienbot/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/ubuntu/repos/lucienbot/.claude/notes/`)
...
2. Glob `/home/ubuntu/repos/lucienbot/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/ubuntu/repos/lucienbot/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/ubuntu/repos/lucienbot/.claude/notes/` — works in any directory
